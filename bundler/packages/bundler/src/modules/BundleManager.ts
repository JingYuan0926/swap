...
export class BundleManager implements IBundleManager {
  ...
  // Cache mapping UserOp hash -> Transaction hash for quick receipt lookup
  private userOpToTxHash: Map<string, string> = new Map()
  // Cache mapping Sender+Nonce -> Transaction hash for robust lookup (handles hash mismatches)
  private senderNonceToTxHash: Map<string, string> = new Map()
  ...
  /**
   * Get cached transaction hash for a UserOp hash
   */
  getCachedTxHash(userOpHash: string): string | undefined {
    return this.userOpToTxHash.get(userOpHash)
  }

  /**
   * Get cached transaction hash by Sender and Nonce
   */
  getCachedTxHashBySenderNonce(sender: string, nonce: string): string | undefined {
    return this.senderNonceToTxHash.get(`${sender.toLowerCase()}-${nonce}`)
  }
  ...
  async sendBundle(userOps: UserOperation[], eip7702Tuples: EIP7702Authorization[], beneficiary: string, storageMap: StorageMap): Promise<SendBundleReturn | undefined> {
    try {
      ...
      // Cache the transaction hash for each UserOp for instant receipt lookup
      for (const userOpHash of hashes) {
        this.userOpToTxHash.set(userOpHash, ret)
        console.log(`📝 Cached tx ${ret} for UserOp ${userOpHash}`)
      }

      // Also cache by sender/nonce
      for (const userOp of userOps) {
        this.senderNonceToTxHash.set(`${userOp.sender.toLowerCase()}-${tostr(userOp.nonce)}`, ret)
      }
