  ...
  async sendUserOperation(
    ...
    await this.mutex.runExclusive(async () => {
      ...
      try {
        ...
        if (!skipValidation) {
          console.log(`🔍 Validating UserOp for sender ${userOp.sender}, nonce ${userOp.nonce}`)
          validationResult = await this.validationManager.validateUserOp(userOp)
          console.log(`✅ Validation passed for ${userOp.sender}`)
        }
        ...
        this.mempoolManager.addUserOp(
          ...
          userOpHash,
          validationResult)
        console.log(`✅ UserOp added to mempool: ${userOpHash}`)
        if (!this.rip7560 || (this.rip7560 && this.useRip7560Mode === 'PUSH')) {
          await this.attemptBundle(false)
        }
      } catch (error: any) {
        console.error(`❌ UserOp validation failed for ${userOp.sender}:`, error.message || error)
        throw error
      }
    })
  }
